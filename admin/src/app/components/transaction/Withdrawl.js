...
const Withdrawl = () => {
  ...
  const update = async (amount, type, userID, txnID, reqID) => {
    ...
    //bank
    var pathUrl = baseUrl+`withdraw/payoutmanualupi`;
    if(type=='upi'){
                ...
                <table className="table">
                  ...
                  <tbody>
                    ...
                    <td>
                          ...
                          {/* <button type="button" className="btn btn-primary" onClick={() => checkStatus(item._id)}><i className="fa fa-eye" aria-hidden="true"></i> status</button> */}
                     



{
    (item.status != "SUCCESS" && item.status != "FAILED" && item.status!="reject")? <button className="mr-1 btn btn-sm btn-danger" onClick={() => withdrowFail(item.txn_id)}>Reject By Admin</button> :''
}

{
    (item.status != "SUCCESS" && item.status != "FAILED" && item.status!="reject")? <button className="ml-1 btn btn-sm btn-warning" onClick={() => withdrowPass(item.txn_id)}>Approve By Admin</button>:''
}
