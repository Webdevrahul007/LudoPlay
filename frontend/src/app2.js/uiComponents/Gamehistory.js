...
const Gamehistory = () => {
  ...
  const Allgames = async (id) => {
    ...
    await axios
      .get(
        baseUrl + `game/history/user/${id}?page=${pageNumber}&_limit=${limit}`,
        { headers }
      )
      .then((res) => {
        let gameHis = [];
        ...
        {game &&
          game.map((card, index) => (
            ...
            >
              ...
              <div
                className={`mx-3 d-flex ${css.list_body}`}
                style={{ width: "70%" }}
              >
                ...
                <div className="d-flex flex-column font-8">
                  ...
                  {/* map the cardData */}
                  <div className={`${css.center_xy} ${css.list_date} `} style={{color: '#2754beb8'}}>
                    <div>
